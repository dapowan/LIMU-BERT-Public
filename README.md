...
LIMU-BERT adopts the principle of the natural language model BERT to effectively capture temporal relations and feature distributions in IMU sensor data. With the representations learned via LIMU-BERT, task-specific models trained with limited labeled samples can achieve superior performances. 
...
- [`config`](./config) : config json files of models and training hyperparameters.
...
- [`classifier.py`](./classifier.py) : run LIMU-GRU that inputs representations learned by LIMU-BERT and outputs labels for target applications.
- [`classifier_bert.py`](./classifier_bert.py) : run LIMU-GRU that inputs raw IMU readings and outputs labels for target applications.
...
- [`utils.py`](./utils.py) : some helper functions for preprocessing data or separating the dataset.

## News
[09/11/2023] We adopted LIMU-BERT in commercial applications and released our latest model, **[LIMU-BERT-X](https://github.com/WANDS-HKUST/LIMU-BERT_Experience)**, which is trained on **1.43 million hours of real-world sensor data** collected from **60K subjects** using **1.1K phone models**. This work has been **accepted to ACM MobiCom 2025**, and you can read the paper **[here](https://dl.acm.org/doi/10.1145/3680207.3765261)**.

[02/10/2023] Our latest work [UniHAR](https://dapowan.github.io/wands_unihar/) is accepted to MobiCom 2023, which is a universal human activity recognition built upon LIMU-BERT.

[03/03/2022] We built a Docker image with the environment to run the code directly. Many thanks to [BryanBo-Cao](https://github.com/BryanBo-Cao).

[20/01/2022] We uploaded trained models (i.e., LIMU-BERT and LIMU-GRU) under the _saved_ folder.

[13/01/2022] We uploaded the four preprocessed datasets under the _dataset_ folder. You can now run our scripts directly.
...
In this option, we can use the image that has installed the required environments for the usage of this repo directly. Once we successfully start a container on this image, we can skip Option2.
...
Locate the folder in the container
```
...
## Dataset
In the [`dataset`](./dataset) folder, we provide four scripts that preprocess the corresponding datasets. Those datasets are widely utilised in previous studies:
...
Each script has a kernel function which reads the raw IMU data and outputs preprocessed data and labels. You can set the sampling rate and window size (sequence length) in the function. It returns two values:
- Data: a numpy array with the shape of (N\*W\*F), N is the number of samples, W is the window size, and F is the number of features (6 or 9).
- Label: a numpy array with the shape of (N\*W\*L), N is the number of samples, W is the window size, and L is the number of label types (e.g., activity and user labels). The detailed label information is provided in [`data_config.json`](./dataset/data_config.json).
...
For example, all data and label are saved as "data_20_120.npy" and "label_20_120.npy" in the settings of LIMU-BERT. And the data and label arrays of the HHAR dataset are saved in the _dataset/hhar_ folder.
...
In implementation, there are three steps to run the code:
...
In the main function of [`pretrain.py`](./pretrain.py), you can set the following parameters:
...
The target task corresponds to the first label in "label_20_120.npy" of the UCI dataset, which is a human activity recognition task defined in [`data_config.json`](./dataset/data_config.json). The train settings are defined in the [`train.json`](./config/train.json).

In the main function of [`classifier.py`](./classifier.py), you can set the following parameters:
...
If you are confused about the above settings, please check Section 4.1.1 in our paper for more details.
...
```
For this command, we will train a composite classifier with the pretrained LIMU-BERT and GRU classifier
, whose settings are defined in the _gru_v2_ of [`classifier.json`](./config/classifier.json). 
...
Note that "v1_v2" defines two model versions, which correspond to the LIMU-BERT and GRU classifier, respectively.
...
In the main function of [`benchmark.py`](./benchmark.py), the parameters are the same as those in [`classifier.py`](./classifier.py).
